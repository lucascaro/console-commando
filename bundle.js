...
const debug = src("console-commando:args");
function parseArgv(argv, merge = Map()) {
    return Map(minimist(argv)).merge(merge);
}
function getValue(o, parsed) {
...
function parseOptions(parsed, options) {
    ...
    return options.map((o) => {
        ...
        if (o.kind === "number") {
            ...
            return Object.assign({}, o, { value: Number(value) });
        }
        if (o.kind === "string") {
            if (!o.multiple) {
                ...
                return Object.assign({}, o, { value: String(value) });
            }
            if (Array.isArray(value)) {
                return Object.assign({}, o, { value: value.map(String) });
            }
            return Object.assign({}, o, { value: [String(value)] });
        }
        return o;
...
function parseArguments(parsed, args) {
    ...
    positional = positional.map(String);
    ...
    return args.map(a => {
        ...
        if (a.kind === "string" && a.multiple) {
            return Object.assign({}, a, { value: positional });
        }
        ...
        if (a.kind === "number") {
            ...
            return Object.assign({}, a, { value: nArg });
        }
        if (a.kind === "string" && !a.multiple) {
...
function getOptionOrParentOption(name, map1, map2) {
...
const debug$1 = src("console-commando:completion");
function completeSubcommands(command, prefix = "") {
    debug$1("complete subcommands", { prefix, command: command.state });
    const names = Array.from(command.state.subCommands.keys());
    return prefix ? names.filter(n => n.startsWith(prefix)) : names;
}
function completeOptions(cmd, prefix = "") {
    debug$1(`complete options with prefix ${prefix}`);
    return flatten(Array.from(cmd.state.options
        .map(o => {
        const completions = [];
...
function completeArguments(cmd) {
...
function getArgCompletions(command, args = []) {
    debug$1("args", args);
    debug$1("command", command.state.name);
    ...
    debug$1("subcommand?", firstArg, subcommand && subcommand.state);
    if (subcommand) {
        const subArgs = args.slice(1);
        debug$1(`complete subcommand '${subcommand.state.name}' with args: ${subArgs}`);
        return [
            ...completeOptions(command),
...
function getCompletions(command) {
    const args = command.state.parsedRuntimeArgs.get("_");
    debug$1("INITIAL ARGS", args);
...
function formatColumns(columns) {
    ...
    const nCols = columns.first().length;
...
function formatOptions(options) {
    const longParam = (o) => (o.long ? `=<${o.kind}>` : "");
...
const debug$2 = src("console-commando:Command");
(function (ReturnValue) {
    ReturnValue[ReturnValue["FAILURE"] = 0] = "FAILURE";
    ReturnValue[ReturnValue["SUCCESS"] = 1] = "SUCCESS";
})(exports.ReturnValue || (exports.ReturnValue = {}));
function withState(initialState) {
    ...
    function withVersion(version) {
        debug$2("adding version:", version);
        return withState(Object.assign({}, cmd.state, { version }));
    }
    function withDescription(description) {
        debug$2("adding description:", description);
        return withState(Object.assign({}, cmd.state, { description }));
    }
    function withOption(definition) {
        if (!cmd.state.parsedRuntimeArgs.isEmpty()) {
            throw new TypeError(`arguments cannot be added after runtime arguments are set: ${definition.name}`);
        }
        debug$2("adding option:", definition);
        ...
        if (map.has(definition.name) ||
            ...
            throw new TypeError(`option already exists: ${definition.name} -${definition.short} --${definition.long}`);
        }
        const options = map.set(definition.name, Object.freeze(definition));
        return withState(Object.assign({}, cmd.state, { options }));
    }
    function withHelp() {
        debug$2("adding auto help.");
    ...
    function withArgument(definition) {
        if (cmd.state.runtimeArgs.size > 0) {
            throw new TypeError(`arguments cannot be added after runtime arguments are set: ${definition.name}`);
        }
        debug$2("adding argument:", definition);
        const map = cmd.state.arguments;
        if (map.has(definition.name)) {
            throw new TypeError(`argument with this name already exists: ${definition.name}`);
        }
        const args = map.set(definition.name, Object.freeze(definition));
        return withState(Object.assign({}, cmd.state, { arguments: args }));
    }
    function withSubCommand(subCommand) {
        debug$2("adding subcommand:", subCommand.state.name);
        ...
        return withState(Object.assign({}, cmd.state, { subCommands }));
    }
    ...
    function getMultiStringOption(name) {
        const opt = getOptionOrParentOption(name, cmd.state.options, cmd.state.parentOptions);
        if (opt.kind !== "string" || !opt.multiple) {
    ...
    function getMultiStringArg(name) {
        const opt = cmd.state.arguments.get(name);
        if (!opt) {
            throw new TypeError(`${name} is not a known option`);
        }
    ...
    function withHandler(fn) {
        return withState(Object.assign({}, cmd.state, { handler: fn }));
    }
    function withPreProcessor(fn) {
        return withState(Object.assign({}, cmd.state, { preProcessor: fn }));
    }
    function showHelp() {
        console.log(formatHelp(cmd.state));
    }
    function withRuntimeArgs(args, parsed) {
        debug$2("adding runtime arguments:", args);
        ...
        try {
            const parsedRuntimeArgs = parseArgv(commandArgs, parsed || cmd.state.parsedRuntimeArgs);
            ...
            debug$2("PARSED", parsedRuntimeArgs);
            return withState(Object.assign({}, cmd.state, { runtimeArgs,
                options,
                parsedRuntimeArgs, arguments: args }));
        }
        catch (e) {
            console.error(colors.red(`\nError: ${e.message}\n`));
            showHelp();
            return process.exit(exports.ReturnValue.FAILURE);
        }
    }
    function run(state = Map()) {
        debug$2("running command:", cmd.state.name);
        const parsedArgs = cmd.state.parsedRuntimeArgs;
        const positionalArgs = cmd.state.parsedRuntimeArgs.get("_", []);
        debug$2("positional args:", positionalArgs);
        ...
        if (!shouldRunSubCommand && helpRequested) {
            cmd.showHelp();
            return Promise.resolve(exports.ReturnValue.SUCCESS);
        }
        if (arg0 === "completion") {
        ...
        if (shouldRunSubCommand) {
            const subCommand = withState(Object.assign({}, cmd.state.subCommands.get(arg0).state, { parentOptions: cmd.state.options.merge(cmd.state.parentOptions), parentArguments: cmd.state.arguments.merge(cmd.state.parentArguments) }));
            const subArgs = parsedArgs.get("_", []);
            return subCommand
        ...
        if (!cmd.state.handler) {
            ...
            cmd.showHelp();
            return Promise.resolve(exports.ReturnValue.FAILURE);
        }
        return Promise.resolve(cmd.state.handler(cmd, runtimeState)).then(retval => retval || exports.ReturnValue.SUCCESS);
    }
...
const debug$3 = src("console-commando:main");
function command(name) {
    debug$3(`creating new command: ${name}`);
    return withState({
        name,
        options: Map(),
        parentOptions: Map(),
        ...
        subCommands: Map(),
        runtimeArgs: List(),
        parsedRuntimeArgs: Map(),
    });
}
function flag(long, short, description, required = false) {
    debug$3("creating option:", { long, short, description, required });
    return {
        long,
        short,
        description,
...
function stringOption(long, short, description, defaultValue, required = false) {
    debug$3("creating option:", {
        long,
        short,
        description,
        defaultValue,
...
function numericOption(long, short, description, defaultValue, required = false) {
    debug$3("creating option:", {
        long,
        short,
        description,
...
function multiStringOption(long, short, description, defaultValue, required = false) {
    debug$3("creating option:", {
        long,
        short,
        description,
...
function stringArg(name, description, defaultValue, required = false) {
    debug$3("creating argument:", { name, description, defaultValue, required });
    return {
        name,
        description,
        required,
...
function numericArg(name, description, defaultValue, required = false) {
    debug$3("creating argument:", { name, description, defaultValue, required });
    return {
        name,
        description,
...
function multiStringArg(name, description, defaultValue, required = false) {
    debug$3("creating argument:", { name, description, defaultValue, required });
    return {
        name,
        description,
...
exports.multiStringOption = multiStringOption;
exports.stringArg = stringArg;
exports.numericArg = numericArg;
exports.multiStringArg = multiStringArg;
