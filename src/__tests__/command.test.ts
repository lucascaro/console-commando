import { command, flag, stringOption, numericOption, multiStringOption } from '../index';
...
describe('multiStringOption', () => {
  ...
  test('can set options for sub commands', (done) => {
    ...
    .withSubCommand(
      ...
      .withHandler((cmd) => {
        expect(cmd.getMultiStringOption('opt')).toEqual(['val1', 'val2', 'val3']);
        expect(cmd.getMultiStringOption('sub-opt')).toEqual(['val4', 'val5', 'val6']);
        done();
      }),
    )
    .withRuntimeArgs([
