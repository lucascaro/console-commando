...
import { command, flag, multiStringOption, numericOption, stringOption } from '..';
...
command(path.basename(__filename))
  ...
  .withDescription(
    'This command has an action and options.',
  )
  ...
  .withOption(multiStringOption('multi', 'm', 'multi options can be specified more than once'))
  .withOption(stringOption('default', 'd', 'arguments can have default values', 'my default'))
  .withHandler((command) => {
    console.log(
    ...
    if (command.getStringOption('string')) {
      ...
      console.log('Option string has value:', command.getStringOption('string'));
    }
